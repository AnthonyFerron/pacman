...
void saveScores() {
    ...
    SDL_Window *window = SDL_CreateWindow("Entrée du nom", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1250, 750, SDL_WINDOW_SHOWN);
    if (window == NULL) {
        ...
        while (SDL_PollEvent(&event)) {
            if (event.type == SDL_QUIT) {
                ...
                if (event.key.keysym.sym == SDLK_RETURN) {
                    ...
                    newEntry.score = gameScore; // Utilisez le score passé en argument
                    running = false;
